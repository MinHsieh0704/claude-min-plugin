...
| `Refs: <內部單號>` | 有人明講內部單號時 | 對應公司內部 bug/suggestion 追蹤單號；可重複多行，一行一個，原樣照抄。**不從分支名或 diff 推斷**，只吃呼叫時給的參數；沒給就在每個 commit 送出前反問一次，預設是不加，見 1.1.3 |
...
- **不由 skill 推斷**：分支名或 diff 內容看起來像單號並不代表就是這次要記的單號，猜錯等於在歷史裡寫下一筆錯誤且無人查核的關聯——比沒有這行更糟。這條沒有變過，只有人明講的單號才會被寫入。
- **但會反問——這是後來推翻的一版設計**：原本連反問都不做，理由是不能打破 commit skill 不中斷的原則。實務上推翻它的是另一條規則：skill 明令**不得 amend**（見 1.6），所以呼叫時忘了帶單號，那個 commit 就永久漏掉這一行，事後無法補救。一次中斷換一筆補不回來的關聯，這個交換划算，因此改為呼叫時沒給就在 commit 前反問一次。呼叫時已經給了的則完全不問，維持原本的快速路徑。
- **反問時不提供候選**：問答只給「不加單號」與自行輸入兩條路，不從分支名、diff 或 `git log` 撈建議值填進選項。理由跟上一條的「不推斷」同源——把猜測放進選項只是把「自動寫錯」降級成「一鍵點錯」，風險性質完全一樣，而使用者對著預填好的號碼更難察覺不對。
- **答案不跨 commit 沿用**：拆成多個 commit 時逐個發問，每個 commit 只帶自己那一題的答案。多個 commit 要共用同一個單號就自己重打一次。這裡刻意選擇多打幾次而非自動沿用，因為會被拆開的正是「彼此不相關」的改動（見 1.1.1 的拆分理由），它們屬於同一張單反而是需要人確認的假設，自動沿用等於憑空生出一筆沒人講過的關聯。呼叫時帶參數的情況是唯一例外：單號在拆分存在之前就只給了一次，skill 沒有依據分派，因此全部套用。
...
`commit` skill（本目錄上一層的 `../SKILL.md`）已更新，會在產生 commit message 時自動判斷並附上以上四種 trailer（`Refs:` 除外——那一種不判斷，只照抄呼叫時給的單號，沒給就逐個 commit 反問一次，見 1.1.3），並在遇到 hook 擋下時照規則重試而非繞過。
