...
4. Generate a conventional commit message **in English** from the diff:
   ...
   - Trailer 4 (only when an internal tracker number was given for this commit): `Refs: <id>`, placed last — after `Fixes:` / `Fixes-Candidates:`.
     ...
     - **A hold stops one commit, not the invocation.** Step 3 splits a diff only when the groups are independent enough to carry their own trailers, so holding the second of three groups has no bearing on the third: the first stands committed and the third proceeds on its own answer. Unstage the held group's paths or hunks: otherwise the next commit swallows them, and even when it is the last group the index should end up naming what still wants review rather than what was already dealt with. The single case that leaves the index untouched is an unsplit invocation, where step 3 staged with `git add -A` and may have swept up changes that were staged before this skill ran and were never its to unstage. Never amend or reset a commit that already exists. What remains in the working tree is then exactly the held group, which is what `git status` should show, and re-invoking the skill is how the human resumes it.
